...
document.addEventListener("DOMContentLoaded", ()=>{
    if (Object.keys(navbar).filter((key)=>navbar[key] == null).length > 0) console.error("FAILED TO LOAD NAVBAR!");
});
...
/* HELPER FUNCTIONS */ const updateBackTotal = ()=>{
    const quantity = (0, _betterhandler.getTotalProductQuantity)();
    console.log("tsdfs");
    if (quantity) {
        navbar.navbarBag.classList.remove("hidden");
        navbar.navbarBag.innerHTML = quantity;
    } else navbar.navbarBag.classList.add("hidden");
};
...
//# sourceMappingURL=checkout.ad585765.js.map
